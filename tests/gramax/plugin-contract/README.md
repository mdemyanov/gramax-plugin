...
## Два дефекта, на которых suite создавался красным

`ac-002` и `ac-004` создавались красными сознательно — это были два реальных дефекта поставленной
...
- **`ac-002`** (негативная часть): живые документы плагина (`README.md`, `skills/writer/SKILL.md`)
  всё ещё учили устаревшему синтаксису тега drawio (`[drawio:...]`, `<Image src=.../>`) вместо
  канонического `<drawio path="..." width="..." height="..."/>`, задокументированного в
  `skills/writer/references/drawio.md`. Недомигрированный тег был дефектом v4.1.0.
- **`ac-004`**: `plugins/gramax/README.md` не нёс WARNING о конфликте триггеров с чужим
  `Agents365-ai/mermaid-skill` (ADR-0008, Решение 6). WARNING отсутствовал полностью.

Оба дефекта закрыты патч-релизом 4.1.1 (задача 4 плана `2026-08-09-test-harness-taxonomy`).
Suite подключён к `scripts/check.sh --full` (шаг 7).
