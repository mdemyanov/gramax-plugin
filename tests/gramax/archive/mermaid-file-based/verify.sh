...
if [ -f "$DSL" ]; then
  ...
  # AC-007: в .mermaid-файле нет markdown- или XML-обёртки (ac-007-no-markup-in-mermaid-file.sh:
  # оригинал ловил именно XML-теги <mermaid>/</mermaid> — их не хватало здесь, из-за чего
  # dirty-diagram-with-markup.mermaid (открывается '<mermaid>') проходил AC-007 незамеченным
  # и ловился только случайно, через AC-002)
  grep -qE '^\s*```|^\s*#{1,6} |\*\*|^<mermaid>|^</mermaid>' "$DSL" \
    && note "AC-007: .mermaid-файл содержит markdown- или XML-разметку — должен нести только DSL"

  # AC-010: нумерованный list-syntax ломает парсер Gramax. Оригинал (ac-010) сознательно
  # не якорил паттерн к началу строки: нарушение живёт внутри текста ноды (A["1. Analyze"]),
  # не в начале сырой строки DSL — якорь '^[[:space:]]*' делал проверку слепой к этому случаю.
  grep -qE '[0-9]+\. ' "$DSL" \
...
if [ -f "$ART" ]; then
  ...
  # AC-009: инлайновый блок не должен быть вырезан молча. Оригинал (ac-009) тестировал две
  # равноправные формы устаревшего блока — fenced ```mermaid и XML <mermaid>...</mermaid>;
  # здесь не хватало второй. '^<mermaid>' (без атрибутов) и '</mermaid>' (закрывающий тег)
  # не пересекаются с новым самозакрывающимся тегом <mermaid path="..." .../>, у которого
  # никогда нет отдельной закрывающей строки и никогда нет '>' сразу после слова mermaid.
  grep -qE '^\s*```mermaid|^<mermaid>|</mermaid>' "$ART" \
    && printf 'NOTE: в статье остался устаревший inline-блок (```mermaid или <mermaid>...</mermaid>) — проверь, что миграция была подтверждена пользователем, а не сделана молча\n'
else
