#!/usr/bin/env bash
# tests/gramax/cross-catalog-retraction/lib/assert.sh
# Копия tests/gramax/writer-consumer-rules/lib/assert.sh (тот же интерфейс, независимая копия —
# suite'ы не шарят lib/ между собой, по прецеденту doc-paths/plugin-contract/catalog-validator/
# mermaid-adoption/writer-consumer-rules).
...
assert_grep_regex_i() {
  local file="$1" pattern="$2" msg="${3:-case-insensitive regex should match in file}"
  if ! grep -qiE "$pattern" "$file" 2>/dev/null; then
    echo "  FAIL: $msg — regex (case-insensitive) '$pattern' not matched in $file" >&2
    FAIL=$((FAIL + 1))
  fi
}
