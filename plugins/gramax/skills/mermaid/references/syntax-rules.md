...
### Формат вставки диаграммы (file-based, v4.0.0+)

Gramax ожидает самозакрывающийся тег-ссылку на внешний `.mermaid`-файл:

```markdown
<mermaid path="./diagram.mermaid" width="800px" height="450px"/>
```

Тег одинаков для XML- и Markdown-каталогов. DSL хранится в отдельном `.mermaid`-файле рядом с md-статьёй. Файл содержит только чистый Mermaid DSL — без обёрток `<mermaid>` или ` ```mermaid `.

**Устаревшие форматы (не используй в новых диаграммах):**

- XML inline: `<mermaid>…</mermaid>` — не отображается корректно в Gramax-рендере.
- Markdown fenced: ` ```mermaid … ``` ` — аналогично.

Если в статье обнаружен inline-блок — skill `gramax:mermaid` предложит миграцию.

### `.doc-root.yaml` syntax

- `syntax: XML` или `syntax: Markdown` — определяет формат остальных блоков в каталоге (admonitions, tables и т.д.).
- Для тега `<mermaid path="…"/>` синтаксис каталога **не влияет** — тег одинаков в обоих случаях.
- Нельзя смешивать XML и Markdown синтаксис в одном каталоге для других блоков.
