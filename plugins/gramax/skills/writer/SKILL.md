...
Полный рецепт (реальный `code:`) → `references/doc-root-schema.md`. Ловушка чужих валидаторов: код-спан `` `[X](Y)` `` (обратные кавычки) внешне похож на markdown-ссылку `[X](Y)` — regex-чекер потребителя может дать ложное срабатывание даже на корректном ❌-примере выше.
