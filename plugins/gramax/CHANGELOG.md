...
Рекурсивное обнаружение `.doc-root.yaml` и типизация обязательных полей (ADR-0020,
FR-120…FR-123): валидатор находит **все** `.doc-root.yaml` в дереве переданного пути и
валидирует каждый как отдельный catalog root полным набором структурных проверок —
семантика Gramax «сканирую весь репозиторий» становится семантикой валидатора.
Регресс-якорь — инцидент 2026-08-13: редактор Gramax не открывался из-за незакавыченного
`title: {{PROJECT_NAME}}` в 24 вложенных `.doc-root.yaml`
(`examples/<профиль>-example/content/`), которые `validate_structure.py` раньше не видел
(exit 0 при невалидном каталоге). Semver — Minor (ADR-0006): новые error-находки меняют
exit-поведение ранее «чистых» каталогов — функциональное изменение, не bug-fix-без-
изменения-поведения (ADR-0020 Решение 7).

### Added

- **Рекурсивное обнаружение catalog root (FR-120)** — `validate()` обходит всё дерево
  переданного пути и валидирует каждый найденный `.doc-root.yaml` как отдельный root;
  файл принадлежит ближайшему предку с `.doc-root.yaml`, поддеревья вложенных root
  исключаются из области ancestor-каталогов (переиспользование границы `in_scope=False`)
  — дефект даёт ровно одну находку (BR-004)
- **Исключения обхода (FR-120)** — явный список: по имени каталога на любой глубине
  `.git`, `.gramax`, `node_modules`; по относительному пути от переданного корня
  `tests/**` и `plugins/gramax/scripts/tests/fixtures/**` (догфудинг, AC-040)
- **Типизация обязательных полей (FR-121)** — для `title`/`language`/`syntax`
  (`docRootRequiredFields`) проверяется не только присутствие, но и тип: значение
  обязано быть **непустой строкой**; dict/list/bool/int/float/date/null/пустая строка →
  error с фактическим типом и исходной строкой файла
- **Негашение диагностики (FR-122)** — ранний `return` при `yaml.YAMLError` в
  существующем `.doc-root.yaml` убран: остальные проверки (минимум `check_placeholders`)
  выполняются даже для root с непарсящимся файлом; сообщение несёт номер строки/колонки
  из pyyaml (`problem_mark`), fallback на `str(e)`
- **Подсказка закавычивания (FR-123)** — если парсинг упал на незакавыченном `{{...}}`,
  сообщение дополняется конкретным исправлением — `title: "{{PROJECT_NAME}}"`;
  parse-error с подсказкой поглощает placeholder-находку того же токена (BR-004)
- **4 acceptance-теста ac-021…ac-024** (suite `tests/gramax/catalog-validator/`) с
  фикстурами и pytest-кейсы в `scripts/tests/test_validate_structure.py` — регресс-якорь
  инцидента: AC-036 (ac-021) на до-FR-120 фикстуре даёт exit 0

Contract: семантика валидации `docRootRequiredFields` (`gramax-catalog-rules.json`)
изменилась — к проверке присутствия ключа добавлена типизация: `title`/`language`/`syntax`
обязаны быть непустой строкой (прецедент маркера — ADR-0012 FR-041).

### Migration notes (с v4.4.0)

Строгость растёт: каталоги, ранее проходившие валидацию с `title: 4.21`, `title: yes`,
`title: {a: b}` или вложенными битыми `.doc-root.yaml`, теперь краснеют (находки —
генуинные дефекты того класса, что роняли редактор Gramax; ADR-0012 G1: известных
потребителей у валидатора нет). Каталоги с копиями `examples/` получают больше находок —
каждый вложенный `.doc-root.yaml` валидируется полностью. Правок в контенте не требуется:
исправление — закавычить плейсхолдер в `title`.
